    ...
    (Argv.localFiles = localFiles.map(
      file => Display.line() && {
        name: process.platform === 'win32'
          ? windowPath((Argv.data.folder.length ? Argv.data.folder + Path.sep : '') + file.replace(Path.dist, ''))
          : (Argv.data.folder.length ? Argv.data.folder + Path.sep : '') + file.replace(Path.dist, ''),
        path: file })) &&
